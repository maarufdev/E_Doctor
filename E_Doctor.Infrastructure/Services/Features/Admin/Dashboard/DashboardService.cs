...
namespace E_Doctor.Infrastructure.Services.Features.Admin.Dashboard;
    ...
    public async Task<Result<GetDashboardDetailsDTO>> GetDashboardDetails()
    {
        ...
        var recentDiagnosisDTO = recentDiagnosis
            .Select(d =>
            {
                ...
                return new GetDashboardRecentDiagnosisDTO(DateTimeHelper.ToLocalShortDateTimeString(dateTime.Value), d.Symptoms, d.IllnessName, d.FullName);
            })
            .ToList();
